...
Let's say we wanted to know more about Asian countries in 2002. First we should go ahead and create a dataframe that consists of just the data we are wanting to look at.
...
Often one will want to sort by more than one column. To do that, instead of passing in a single column, we pass in a list of the columns we want to sort on. We can also control whether we would like each column to be sorted in ascending or descending order by passing in the respective list of boolean values in the `ascending=` parameter.
...
```
<br>
Another way is to call the pandas function `apply()` in which other functions can be passed for a given column. In the example below, the `len()` function calculates the number of characters for each row (i.e. for each country name in the "country" column) and returns the number of characters for that row in a new column:
