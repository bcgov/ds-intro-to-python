...
Depending on what we want to do with `countries_2007` going forward, we might want to create a new "default" index for it instead of the existing one. That's where the `.reset_index()` function comes into play. 
...
Let's use the iloc() method to look at the positional content of this `countries_2007` object and confirm that Japan shows up in row 0 as we expect. 
...
1. Select only the rows where the country is equal to Ireland AND where the year is 1997 or greater
