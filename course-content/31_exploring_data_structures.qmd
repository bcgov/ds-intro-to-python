...
```
<br>
Another way is to call the pandas function `apply()` in which other functions can be passed for a given column. In the example below, the `len()` function is applied to each row in the "country" column and a new column with the number of characters for that row is returned:
