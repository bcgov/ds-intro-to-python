...
In our example below, we will create a new dataframe called "narrow_country_metrics", and we are primarily interested in getting the population column from the original dataset, but we want to have just year, country, and continent in our new dataframe also. So the dataset will be more narrow than the one it was created from. 

There are two ways to approach this. The first is to identify all the columns you want to keep. 
...
The second way is use the pandas `.drop()` method to drop the columns from the original object that you do not want. We can achieve the same result as the example above by simply dropping two variables instead of naming the other four. With this method, we also need to add the specification `axis = 1`, which indicates that it is columns being referenced for being dropped. 
...
It enables us to select a particular set in the in the dataset that belongs to a particular row or column, accoring to the integer based numbering system as shown in the image below:
...
By referring to these locations, we can use `iloc()` to retrieve the exact cells we want to see. For example, if we just want to say, the row of data corresponding to the integer value 5, we would run the following code:
...
It is a flexible tool that can be very helpful as you build your code. 
<br>
...
Sometimes you will want to invoke a number of calls sequentially to some object, with each one in turn performing some action on it. 
...
country_size = pd.read_csv(countrysize_url, encoding= 'unicode_escape')
country_size.info()
...
```{python}
combined_df = country_metrics.merge(countrysize, left_on='country', right_on='nation', how='left')
combined_df.head(2)
```
