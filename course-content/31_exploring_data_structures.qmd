...
While pandas makes it possible to join two dataframes on their index, joining on a particular "key" column is also possible. Joining on a key column is likely more intuitive for those familiar with merging with vlookup in Excel or joining tables with SQL. So let's go ahead and join this based on the "country" column from the original dataset and "nation" from the second one. In database management language, we are going to do a left join (perhaps the most common type of join). 

:::{.callout-tip}

To Merge or to Join?
In pandas, there are two methods we can use to bring these datasets together, namely the `.merge()` and the `.join()` functions. Both can accomplish many of the same tasks and you likely will come across both. `merge()` is a more generalized version of `join(),` and allows for more complicated combinations of data. To keep things simple, we will start with just the `join()` method here, but keep in mind that a lot of the functionality will carry over to `merge()`!
:::
...
Your director has come back, yet again, and wants to know what the top five countries were in terms of GDP per capita in 2007. 
