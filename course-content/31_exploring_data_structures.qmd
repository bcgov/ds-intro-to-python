...
When we want to look at just selected rows (i.e. select rows that have certain values within a given column) we can supply a condition that must be met for a given row in that column.   To do this, we must use one of the following *comparison operators* also called *relational operators*:
...
<br>
Let's try an example. The most commonly used relational operator is likely the equal to `==` so in the example below we have a statement that pandas evaluates line by line in the dataframe as to whether it corresponds to a boolean value of `True` or `False`. 
...
*Logical operators in pandas and python*
...
However, when we ask pandas to evaluate whether a set of logical relations exist within a pandas series object, we must use pandas *bitwise* logical operators:

 - and `&`
 - or `|`
 - not `~`

When combining multiple conditional statements in a pandas series object, each condition must be surrounded by parentheses `()` within the square brackets `[]`. In the example below, we use an `&` to indicate that both conditions must be true for a row to be retured:


```{python}
filtered_country_metrics = country_metrics[
    (country_metrics['year'] == 1972) & (country_metrics['country'] == 'Albania')
    ]
...
In the example above, there are many rows that satisfy one or the other condition.
...
Let's say we wanted to know more about Asian countries in 2007. First we should go ahead and create a dataframe that consists of just the data we are wanting to look at.

```{python}
countries_2007 = country_metrics[
    (country_metrics['year'] >= 2007) & \
...
```{python}
countries_2007 = countries_2007.sort_values('lifeExp', ascending=False)
countries_2007.head()
```

That was pretty straightforward and it helps us get more insights. Looks like Japan is the Asian country with the highest life expectancy in 2007.
...
Depending on what we want to do with `countries_2007` going forward, we might want to create a new "default" index for it instead of the existing one. That's where the .reset_index() call comes into play. 

In the code below, we reset the index.

```{python}
countries_2007 = countries_2007.reset_index(drop=False)
countries_2007.head()
```
...
Let's use the iloc() method to look at the positional content of this `countries_2007` dataframe and confirm that Japan shows up in row 0 as we expect. 

```{python}
countries_2007.iloc[0:4,0:5]
```
...
Fortunately, if we want to look at several aggregations at once, we can do that too. To extend our example above, we would specify the "continent" column in the `.groupby()` function, then pass additional aggregation functions (in our case, we will add  "mean", "min", and "max") as a dictionary within the `.agg()` function. This dictionary takes the column that we are aggregating - in this case life expectancy - as a key and the aggregation functions as its value. We also add a line of code that gives each of the columns a new name with the .columns() function.
...
This looks more like the structure of the dataframes we know already. 
...
Using what we learned about how to select rows, we should limit the dataframe to rows where the year is greater than or equal to 1997. Next we should create a multi-column call to the `.groupby()` function. Finally we should select some aggregations such as mean and max among others could make sense here.
