...
However, when we ask pandas to evaluate whether a set of logical relations exist within a pandas series object, we must use pandas logical operators:
...
In the example above, there is only one row of data that matches the condition. We can use the `|` which indicates that if *either or both* of the conditions are true for a given row, that row is returned into the object:
...
```{python}
#| scrolled: true
chained_df = country_metrics[(country_metrics['country'] == 'Canada') & \
