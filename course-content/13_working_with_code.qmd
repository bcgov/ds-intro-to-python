...
What is the main risk to a project that you can think of if data are not kept well-organized? 
...
Existing or future team members looking through project documents will not understand how data flows, so interpreting and/or fixing it will prove challenging.  
...
Output can always be reproduced if the data itself and details about how the data is transformed (cleaned, munged, analyzed, etc.) is available. The loss of things that can be generated from data and code is not an serious risk to the project.
...
```{python}
a = print("hello")  # Correct, a space on either side of the assignment operator 

a=print("hello")    # Wrong
```

```{python}
print(2)  # Correct, no space between the function call and a parenthesis
