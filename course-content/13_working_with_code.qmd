...
Most data projects consist of some version of a) finding some data, b) cleaning it up, and c) generating some insights. In an Excel world, all three stages often happen in a single document. Albeit usually over many tabs, and almost always with little documentation. If the project is purely "ask a one time question and get a one time answer and never need to look at the file again", then this approach is arguably efficient and effective. But what if we want to update the insights over time or apply the question to a different dataset? What is someone wants to reproduce what we did?  How about verify if what we did was valid? Then we have to admit that we have something to work from that is neither efficient nor effective. 
...
Because you should be able to take the original data as well as the scripts to process it into something useful, you can recreate the outputs at any time. So while we might have a place where we store the processed data, we can also treat outputs as disposable. 

It is a bit like having a kitchen pantry with ingredients for making a cake along with the recipe to guide us through the steps to make it. We should be able to make the same cake over and over again with a stocked pantry and access to the recipe. But if we lose either the pantry or the recipe, we won't be able to make the cake. And if you are a chef, that is kind of a big deal.

Below are some good simple file organizing practices:

Create a single name for your project and then a number of subdirectories as you develop contents. For example, let's say we have a project that is about analyzing migration flows in BC. Here's how you could structure your work, so that it stays well organized:

/MigrationFlowsBC       *the name of the project, without spaces, consistent naming approach with other projects*
...
-	Reading/getting help in the VS Code environment
-	Where to find things
-	Troubleshooting/Bug detection
