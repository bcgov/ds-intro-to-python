...
Below we apply it to the entire datframe to get a cell by cell view:
...
```{python}
reduced_rows = movie_ratings.dropna(how='all')
reduced_rows.head(11)
```

We still have three raters who have given us ratings for each movie. The dataset is nice and clean now, although it has come at a pretty big cost, losing 7 rows of data! If we go in the other direction, and insert a parameter of `how='all'`, then we have the opposite problem, with *no rows* being eliminated (as there were no instances where all of the columns contained missing data for a given row)! 
...
With the data we just looked at, it made sense to apply techniques to the entire dataframe. Typically, however, it is required to have different strategies for different parts of the dataset. For example, looking at individual columns and applying 


```{python}
nonemo = movie_ratings['Finding Nemo'].dropna()
nonemo.head()
```
...
`Inplace` is an argument used in different functions such as `dropna()`, `fillna()`, `interpolate()` and many more. The default value of this attribute is `False` and it returns the copy of the object. When the argument is `inplace=True`, whatever changes are made by the given function make changes to the actual object, not just to a copy of it!

Starting from scratch, let's get the data from the source:

```{python}
# Create an identical dataframe to the original 
movie_ratings = pd.read_csv('../data/MovieRatingsShort.csv')
```
...
But when we run the same code except with the `inplace=True` parameter, the dropped data is gone!
...
It is therefore important when writing your cleaning code that you use this paramater with intention. 
:::
...
Given that it is for day by day temperature data, it is reasonable to think that the temperature for the missing days would be similar to the day preceding or coming after it. So either bfill or ffill parameter choices of the `fillna()` method could be workable. 
...
That feels like the best solution so far! And this only scratches the surface of how we could fill these empty spaces. 

Similar to `fillna()` is a method called `interpolate()`, which takes the average of the values above and below the missing values. 
...
Unfortunately you notice that several of the days have data missing. What method(s) would you use to clean it up to you have usable data for presentation?  Write code that implements your solution. 
...
With this dataset we've called there are a few columns that we would like to make changes to so that it will be easier to analyze the data in the way we want. The `value.counts()` method is going to be useful to you over and over. 
...
```{python}
interpolate_temps = city_temps.interpolate(method="time")
interpolate_temps.info()
```
...
city_temps.Date = pd.to_datetime(city_temps.Date.str[0])
city_temps.dtypes
