...
Although the content is still in object form, the pandas `to_numeric()` function is able to interpret the 1s and 0s as integers and transform the series into an int64 datatype.
...
Here are a couple more pandas functions to mention:

* `get_dummies()` converts categorical variable into dummy/indicator variables. It 

* `factorize()` encodes the object as an enumerated type or cateogorical variable

And from the world of machine learning, from the sci-kit learn library, called `onehotencoder()`, which is also widely used. 
