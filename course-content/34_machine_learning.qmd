...
Scikit-learn - also known as sklearn - is an open-source machine learning library for the Python programming language and is the library we will look at in this tutorial. It is built on top of other popular Python libraries such as NumPy and pandas, and it provides a wide range of tools and algorithms for tasks such as classification, regression, clustering, and dimensionality reduction. 
...
- size of the party
 ...
 We already know that there is no missing data, so that's great.  Let's run the `sample()` method to get a good balanced initial feel for the data in the dataframe. 
...
With our tip predictor project, there are two approaches we could take and we must decide which one to take at the outset. One is to predict the *actual size of the tip* one would expect to get. This is what would be known as a regression type problem. The other way is to define *what constitutes a bigger tip* from the outset and to predict how likely you would be to get that. This is called a classification type of problem. There is no correct answer, is use-case dependent, and each have their pros and cons. 
...
The next step is to define which variables are to be considered the independent variables (or features, in machine learning terms) and which singular variable is the target variable that we want to predict. Otherwise, the program won't know what variables are used to predict what. 
...
The `train_test_split()` method takes the X and y that we defined above, and splits it randomly according to the relative size we want the test sample to be compared to the total sample size. In our example, we have chosen for the test sample to be 40.9% of the entire dataset.  
...
```
<br>
From the code above, we can see that the 1 means that the model predicts the case to be a big tipper and 0 predicts that the case will not be a big tipper.  

Each of the predictions is based on a discrete probability:

```{python}
y_predict_proba[:5] # estimated probability
```

We can see that for a prediction to be 1, it has to have a probability of .5 or greater.
...
The confusion matrix shows how many guesses the model gets "right", that is, the actual is true when we guess true (TP) and false when we guess false (TN). The remaining two quadrants show the errors. Type I error is the false positive error (FP), where the model guesses true but reality is false. Type II error is where where the model guesses false but the reality is true (FN).
<br>
<br>
Another widely used report is the classification report, which shows precision and recall, two widely reported metrics in machine learning. 
...
It depends! If your goal is to try to make sure you can get a big tip each time you choose a customer, then precision is better. If your goal is wanting to avoid missing out on getting a big tipper, then recall is likely better. 
:::


So there we go, we have nice small proof of concept that we can likely develop a tool that will be helpful to meeting our needs. The next step could be to refine some of the code (e.g. to eliminate redundent columns) and to "tune the parameters" of the model, also creating test and train datasets of different sizes to optimize it. We might also want to improve the model using different estimators (e.g. something other than LogisticRegression) or perhaps several models together! The development and refinement process can go on for some time before ultimately the development of a new data pipeline to automate the process and make it usable for end users. 
