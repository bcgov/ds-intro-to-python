...
We can see for each case a pair of probabilities, one for "0" and the other for "1". For example, for the first set of diners in our test dataset, they had a 48% probability of leaving a low tip, while a 52% probability of leaving a large tip. This detail helps us understand how the decisions with the `predict()` method were made. 


## Evaluating

Ok. We have our predictions... but are they good? Of course, what "good" means needs a bit of thought. There are many performance measures in data science and there are considerations based on the use cases themselves. How we would evaluate performance of an algorithm that attempts to diagnose serious disease would be different than if we were evaluating one that identified whether an email was spam or not. In the case of disease classification, we would not want a high false negative rate, and thus missing out on important treatment for people who actually have the disease. On the other hand with spam filters, it would be okay to miss some spam, but it would be worse to have a high false positive rate, and start chucking out real emails with possible spam. 
