...
```{python}
condition = df.profit == df.groupby('unit').profit.transform('max')
...
current_year = date(1942, 8, 1)
previous_year = date(1941, 8, 1)
next_year = date(1943, 8, 1)

# filter the date column
dowjones[dowjones.Date.isin([current_year, previous_year, next_year])]
```

If we look at these three lines, we see that the `year_prior` and `year_after` columns do in fact give us the corresponding values from the initial dataset. And if we plot this, we will see three copies of the data, but shifted by a year:
